...
pub const GCM_SIV_BLOCK_SIZE: usize = 64 * 1024;

pub const GCM_SIV_TAG_SIZE: usize = 16;

pub const GCM_SIV_FRAME_SIZE: usize = GCM_SIV_BLOCK_SIZE + GCM_SIV_TAG_SIZE;

type Aes256GcmSivCipher = aes_gcm_siv::Aes256GcmSiv;

pub struct AesGcmSiv {
    cipher: Aes256GcmSivCipher,
...
impl AesGcmSiv {
    fn derive_key(password: &[u8]) -> [u8; 32] {
        ...
        hasher.update(b"aesgcmsiv-key-v1");
    ...
    fn boxed_new(password: &[u8]) -> Box<Self> {
        ...
        let cipher: Aes256GcmSivCipher =
            Aes256GcmSivCipher::new_from_slice(&key).expect("32-byte key");
...
impl Cipher for AesGcmSiv {
    ...
    fn plaintext_len_of(&self, cipher_len: u64) -> Option<u64> {
        ...
        let fs: u64 = GCM_SIV_FRAME_SIZE as u64;
        let bs: u64 = GCM_SIV_BLOCK_SIZE as u64;
        let tag: u64 = GCM_SIV_TAG_SIZE as u64;
        let n: u64 = data / fs;
        let r: u64 = data % fs;
        if r == 0 {
            Some(n * bs)
...
impl BlockCipher for AesGcmSiv {
